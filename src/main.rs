...
async fn scan_port_tcp(target: IpAddr, port: u16, timeout: u64) {
...
async fn scan(target: IpAddr, full: bool, concurrency: usize, timeout: u64) {
    ...
    ports
        // .for_each_concurrent(concurrency, |port| scan_port_tcp(target, port, timeout))
        .for_each(|port| async move {
            scan_port_tcp(target, port, timeout).await;
        })
        .await;
    // .for_each(|port| scan_port_tcp(target, port, timeout))
    // .await;
}
